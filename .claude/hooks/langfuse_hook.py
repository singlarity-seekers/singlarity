...
def save_state(state: Dict[str, Any]) -> None:
    try:
        ...
        log.debug(f"save_state failed: {e}")
...
def read_new_jsonl(transcript_path: Path, ss: SessionState) -> Tuple[List[Dict[str, Any]], SessionState]:
    ...
    try:
        ...
        log.debug(f"read_new_jsonl failed: {e}")
        return [], ss
...
def main() -> int:
    ...
    if not session_id or not transcript_path:
        ...
        log.debug("Missing session_id or transcript_path from hook payload; exiting.")
        return 0

    if not transcript_path.exists():
        log.debug(f"Transcript path does not exist: {transcript_path}")
        return 0
    ...
    try:
        with FileLock(LOCK_FILE):
            ...
            for t in turns:
                ...
                try:
                    ...
                    log.debug(f"emit_turn failed: {e}")
        ...
        dur = time.time() - start
        log.info(f"Processed {emitted} turns in {dur:.2f}s (session={session_id})")
        return 0

    except Exception as e:
        log.debug(f"Unexpected failure: {e}")
        return 0
