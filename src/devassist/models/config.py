...
class SourceConfig(BaseModel):
    ...
    Use ClientConfig.source_configs instead.
    """
    enabled: bool = Field(default=True, description="Whether source is enabled")
    credentials: dict[str, Any] = Field(default_factory=dict, description="Source credentials")
    settings: dict[str, Any] = Field(default_factory=dict, description="Source settings")


class AIConfig(BaseModel):
    """Legacy AI configuration (deprecated).

    Use ClientConfig.ai_model and ClientConfig.ai_timeout_seconds instead.
    """
    ...
    model: str = Field(default="gemini-1.5-flash", description="Model ID")
    timeout_seconds: int = Field(default=60, description="Request timeout")


class AppConfig(BaseModel):
    """Legacy application configuration (deprecated).

    Use ClientConfig instead.
    """
    workspace_dir: str = Field(default="~/.devassist", description="Workspace directory")
    ai: AIConfig = Field(default_factory=AIConfig, description="AI configuration")
    sources: dict[str, Any] = Field(default_factory=dict, description="Source configurations")